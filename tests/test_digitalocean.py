...
def test_fetch_cached_json_with_cache():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch(
            "time.time", return_value=cache_file.stat().st_mtime + 1800
        ):  # 30 minutes later
            result = llm_digitalocean.fetch_cached_json(
                url="https://example.com/api", path=cache_file, cache_timeout=3600
            )
...
def test_fetch_cached_json_expired_cache(mock_get):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch(
            "time.time", return_value=cache_file.stat().st_mtime + 7200
        ):  # 2 hours later
            result = llm_digitalocean.fetch_cached_json(
                url="https://example.com/api", path=cache_file, cache_timeout=3600
            )
...
def test_register_models_with_key(mock_get_headers, mock_get_models, mock_get_key):
    ...
    mock_get_models.return_value = [
        {"id": "test-model", "supports_schema": False, "supports_vision": False}
    ]
